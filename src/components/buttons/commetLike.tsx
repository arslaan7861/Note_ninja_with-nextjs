...
export function DislikeButton({
  ...
  const handleClick: React.MouseEventHandler<HTMLButtonElement> = async () => {
    try {
      ...
      setState({
        active: resp.disliked,
        count: resp.disliked ? state.count + 1 : state.count - 1,
      });
    } catch (error) {
  ...
  return (
    <button onClick={handleClick} className="h-full w-min relative flex gap-1">
      <svg
      ...
      {state.count > 0 && (
        <span className="text-sm text-secondary_text">{state.count}</span>
      )}
    </button>
  );
}
export function LikeButton({
  ...
  const [state, setState] = useState({ active: liked, count: likesCount });
  const router = useRouter();
  const handleClick: React.MouseEventHandler<HTMLButtonElement> = async () => {
    try {
      ...
      setState({
        active: resp.liked,
        count: resp.liked ? state.count + 1 : state.count - 1,
      });
    } catch (error) {
